    ...
    const fetchAlbums = async () => {
      ...
      try {
        ...
        if (query) {
          ...
          response = await fetch(
            'https://api.spotify.com/v1/browse/new-releases?offset=0&limit=14&locale=en-US',
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
        ...
        const data = await response.json();
        const albumsData = (
          query ? data.albums.items : data.albums.items
        ).filter(album => album !== null && album !== undefined);

        // 调试日志：记录从API获取的原始数据
        console.log(
          'Raw albums data from Spotify API:',
          albumsData.slice(0, 3)
        ); // 只显示前3个避免日志过多
